  ...
  function ExtendedChance (seed) {
    ...
    function installMapFunction (generator) {
      generator.map = function (f) {
        ...
        if (generator.grow) {
          mapGenerator.grow = function (value) {
            if (value === lastMappedValue) {
              return generator.grow(lastValue).map(f)
            } else {
              return mapGenerator
            }
          }
    ...
    var expanders = {
      ...
      string: function (generator, data) {
        ...
        var count = length || that.natural({ min: 5, max: 20 })
        var characterGenerator = that.character(options)

        return generator.map(function (text) {
          var margin = Math.max(Math.floor(Math.min((text.length - data.length) / 2, data.length * 0.3)), 0)
